...
class CategoryController extends Controller
{
   /**
     ...
     * @Route("/{page}", name="category_paginated", requirements={"page":"\d+"})
     */
    public function showCategoryAdmin(CategoryRepository $categoryRepo)
    {
        $categoryList = $categoryRepo ->findAll();
        return $this->render('admin/category_dashboard.html.twig', ['produits' => $categoryList]);
    }
    ...
    public function editCategory(Request $request, ObjectManager $manager, Category $category = null)
    {
        ...
        else
        {
           ...
           $category->setImage(new File($category->getImage()));
           $oldImage2 = $category->getImage2();
           $category->setImage2(new File($category->getImage2()));
           $oldImage3 = $category->getImage3();
           $category->setImage3(new File($category->getImage3()));
           $group = 'edition';
        }
